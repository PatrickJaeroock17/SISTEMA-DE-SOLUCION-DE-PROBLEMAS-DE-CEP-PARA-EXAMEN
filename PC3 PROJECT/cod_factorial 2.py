...
# El símbolo '*' en OLS incluye los efectos principales y la interacción
modelo = ols("Respuesta ~ C(Fila) * C(Columna)", data=df).fit()
tabla_anova = anova_lm(modelo, typ=1)

# Renombrar índices para mayor claridad en la consola (¡EL ORDEN IMPORTA!)
tabla_anova.index = [
    idx.replace("C(Fila):C(Columna)", "Interaccion_AB")
       .replace("C(Fila)", "Filas")
...
# Une los textos de Fila y Columna (Ej. "1 - M1")
df["Interaccion_AB"] = df["Fila"].astype(str) + " - " + df["Columna"].astype(str)
...
for factor_label, col_df in [("Filas", "Fila"), ("Columnas", "Columna"), ("Interaccion_AB", "Interaccion_AB")]:
    if factor_label in tabla_anova.index:
        ...
        if pd.notna(p):
            ...
            # Guardamos el factor (o la interacción) en la lista si es significativo
            if p < alpha:
...
def imprimir_tukey_factorial(df_datos, col_factor, tabla_anova_res, alpha=0.05):
    print(f"\n{'=' * 60}")
    print(f"  PRUEBA DE TUKEY HSD - {col_factor.upper()} (alpha = {alpha})")
    print(f"{'=' * 60}")
    ...
    # El algoritmo calcula automáticamente si es un efecto principal (b*n) o interacción (n)
...
if factores_sig:
    ...
    print("  >> Ningún efecto resultó significativo. No se aplica Tukey.")
