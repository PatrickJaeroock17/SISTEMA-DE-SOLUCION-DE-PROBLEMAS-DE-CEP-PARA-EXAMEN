...
# ... (tu código anterior donde renombras los índices)

# --- INICIO DE LA CORRECCIÓN PARA DCL INCOMPLETO ---
if a == "INCOMPLETO":
    # 1. Restar 1 al Grado de Libertad del Error (Residual)
    tabla_anova.loc["Residual", "df"] -= 1
    
    # 2. Recalcular el Cuadrado Medio (mean_sq) del Error
    nuevo_gl_error = tabla_anova.loc["Residual", "df"]
    nuevo_mse = tabla_anova.loc["Residual", "sum_sq"] / nuevo_gl_error
    tabla_anova.loc["Residual", "mean_sq"] = nuevo_mse
    
    # 3. Recalcular F y P-values para todos los factores usando el nuevo MSE
    for factor in tabla_anova.index:
        if factor != "Residual":
            # Nuevo F = CM del factor / Nuevo CM del Error
            nuevo_f = tabla_anova.loc[factor, "mean_sq"] / nuevo_mse
            tabla_anova.loc[factor, "F"] = nuevo_f
            
            # Nuevo P-value usando scipy.stats.f.sf(x, df1, df2)
            gl_factor = tabla_anova.loc[factor, "df"]
            nuevo_p = f.sf(nuevo_f, gl_factor, nuevo_gl_error)
            tabla_anova.loc[factor, "PR(>F)"] = nuevo_p
# --- FIN DE LA CORRECCIÓN ---

print(tabla_anova.round(4).to_string())

# ... (tu código continúa normal hacia el resumen de significancia)
